...
export default function TFFPortal() {
  ...
  // Chumpions standings (1XI only) with BYE = +1pt
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {Array.from({length: currentWeek}, (_,i)=>i+1).map(week => {
                  ...
                  const scores = wk.filter((m:Match)=>!m.bye && typeof m.homeScore==='number').flatMap((m:Match)=>[m.homeScore!, m.awayScore!]);
